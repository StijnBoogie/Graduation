...
function App() {
  ...
  // Home Screen will be empty (no battery) until the user taps Add and
  // enters their code.
  ...
  return (
    <div style={{
      ...
      {/* Brand mark. Hidden on lockscreen + setup so they keep
          their immersive full-bleed presentation. Rendered at the BOTTOM
          (after the content) — see below. */}
      ...
      {/* Brand mark pinned to the bottom of the page, on the same
          background as the rest of the page (no separate coloured bar). */}
      ...
      {/* Moderator-only scenario switcher. Small and faint so the
          participant barely registers it. Rendered on every in-app screen
          (overview, lockscreen, detail) so the moderator can both jump
          scenarios AND push a notification at any point. Hidden on setup. */}
      {!isSetup && <ModeratorSwitcher />}
    </div>
  );
}
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 13.5, color: 'rgba(0,0,0,0.55)', lineHeight: 1.4, marginBottom: 18 }}>
          You'll find this code on the packaging of your HeatGuard sensor.
        </div>
        <form
...
const MOD_SCENARIO_LINKS = {
  ...
  alert1: { href: 'orange1.html', label: 'Small anomaly',     colour: '#E89A33' },
  alert2: { href: 'orange2.html', label: 'Elevated risk',     colour: '#C77700' },
  red1:   { href: 'red1.html',    label: 'Developing danger', colour: '#D9342A' },
  red2:   { href: 'red2.html',    label: 'Imminent risk',     colour: '#9C0F1B' },
};
...
function ModeratorSwitcher() {
  ...
  // A plain jump lands on the home/overview (clears any stale lockscreen
  // state) so the moderator sees the live app, not a leftover notification.
  const navigate = (href) => {
    setOpen(false);
    try { localStorage.setItem('ess.screen', 'overview'); } catch (_) {}
    window.location.href = href;
  };

  // Simulate a push notification for a scenario: force the target page to
  // boot into the lockscreen, where the scenario's notification is shown.
  // Tapping the notification then opens the app exactly as it would in the
  // field. This mirrors the moderator sending an NTFY push and the
  // participant seeing it land on their lock screen.
  const notify = (href) => {
    setOpen(false);
    try { localStorage.setItem('ess.screen', 'lock'); } catch (_) {}
    window.location.href = href;
  };
      ...
      {open && (
        <div style={{
          ...
          <div style={{
            ...
            padding: '4px 8px 2px', fontWeight: 600,
          }}>Moderator</div>
          <div style={{
            fontSize: 8.5, color: 'rgba(255,255,255,0.3)',
            letterSpacing: 0.2, padding: '0 8px 6px', lineHeight: 1.3,
          }}>Tap row to jump · bell to push notification</div>
          {MOD_SCENARIO_ORDER.map(k => {
            const s = MOD_SCENARIO_LINKS[k];
            const active = k === current;
            const canNotify = k !== 'ok';
            return (
              <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <button
                  type="button"
                  onClick={() => canNotify && notify(s.href)}
                  disabled={!canNotify}
                  title={canNotify ? `Push ${s.label} notification` : 'No notification for Normal'}
                  aria-label={canNotify ? `Push ${s.label} notification` : 'No notification'}
                  style={{
                    width: 28, height: 28, flexShrink: 0,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    borderRadius: 8, border: 'none', padding: 0,
                    background: canNotify ? 'rgba(255,255,255,0.06)' : 'transparent',
                    cursor: canNotify ? 'pointer' : 'default',
                    opacity: canNotify ? 1 : 0.18,
                  }}
                >
                  <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                    <path d="M8 2.2c-2.2 0-3.6 1.6-3.6 3.8 0 3.4-1.1 4.3-1.6 4.8-.2.2-.1.5.2.5h10c.3 0 .4-.3.2-.5-.5-.5-1.6-1.4-1.6-4.8C11.6 3.8 10.2 2.2 8 2.2z"
                      stroke={canNotify ? s.colour : '#fff'} strokeWidth="1.4" strokeLinejoin="round"/>
                    <path d="M6.6 13c.2.7.8 1.1 1.4 1.1.6 0 1.2-.4 1.4-1.1"
                      stroke={canNotify ? s.colour : '#fff'} strokeWidth="1.4" strokeLinecap="round"/>
                  </svg>
                </button>
              </div>
            );
