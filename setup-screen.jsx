...
function SetupScreen({ onFinish }) {
  ...
  // Listen for the iframe's "Start Safety" button. When it fires, finish
  // setup the same way the Skip button does — navigating to the home
  // screen with the empty (no-battery) state ready.
  React.useEffect(() => {
    const handler = (e) => {
      const data = e && e.data;
      if (data && data.type === 'heatguard:start-safety') {
        onFinish && onFinish();
      }
    };
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.1 }}>
            ...
            <span style={{ fontSize: 11, color: 'rgba(0,0,0,0.55)', marginTop: 2, letterSpacing: -0.05 }}>Apply the HeatGuard patch to your e-bike battery</span>
          </div>
        </div>
        <button
          onClick={onFinish}
      ...
      <iframe
        title="HeatGuard sensor application walkthrough"
